...
- (void)setCell: (UITableViewCell *) cell
    ...
    UIImageView *videoPreview = (UIImageView *)[cell viewWithTag:1102];
    ...
    videoPreview.image = [UtilityViewController getImageFromStringURL:[dictStory objectForKey:@"video_preview_pic"]];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if([segue.identifier isEqualToString:@"showBrowseDetail"]){
        BrowseDetailViewController *browseDetailViewCtrl = (BrowseDetailViewController *)segue.destinationViewController;
        
        NSIndexPath *indexPath = [self.browseTableView indexPathForSelectedRow];
        NSDictionary *dictStory = [self.aStories objectAtIndex:indexPath.row];
        
        browseDetailViewCtrl.story_id = [dictStory objectForKey:@"story_id"];
    }
}
