...
-(void)getFollowers:(NSString *)userId
  ...
  filtered_followed:(Boolean)filtered
            success:(APISuccessBlock)success
            failure:(APIFailureBlock)failure{
    NSString *strPath = [NSString stringWithFormat:@"%@/%@?start=%@&limit=%@&filter_followed=%hhu", GRAPH_FOLLOWERS, userId, start, limit, filtered];
