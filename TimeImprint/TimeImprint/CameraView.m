...
@property (nonatomic, assign) BOOL finishEarly;
@end
...
//    self.playBtnBackground.layer.borderWidth = 4;
    ...
    self.recordingIsOkayBtnBackground = [[UIView alloc]initWithFrame:CGRectMake(btnOverView.frame.size.width-140, btnOverView.frame.size.height/2-25, 50, 50)];
    ...
    [library enumerateGroupsWithTypes:ALAssetsGroupSavedPhotos usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
        ...
        [group enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *alAsset, NSUInteger index, BOOL *innerStop) {
            ...
            if (alAsset) {
                ...
                [[openCameraRollBtn imageView] setContentMode: UIViewContentModeScaleAspectFit];
...
-(void)increaseProgressView {
    ...
    [UIView animateWithDuration:1 animations:^{
        ...
        if (_videoProgressView.frame.size.height >= 200) {
...
-(void)saveToPhoneAlbumWithPath:(NSString *)videoPath {
    ...
    [CameraView shared].camRollURL = [NSURL URLWithString:[@"file://" stringByAppendingString:videoPath]];
    ALAssetsLibrary *library = [[ALAssetsLibrary alloc] init];
