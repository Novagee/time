...
- (void)awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hidePopoverLayer:)
                                                 name:@"HidePopoverLayer"
                                               object:nil];
}
...
- (IBAction)optionTapped:(id)sender {
    ...
    controller.story_id = self.story_id;
    //TODO: HAVE TO ADJUST POPOVER POSITION
    self.popover = [[FPPopoverController alloc] initWithViewController:controller];
    self.popover.contentSize = CGSizeMake([[UIScreen mainScreen] bounds].size.width, 90);
...
- (void)initStory:(Story *)story{
    ...
    self.postTitleLabel.text = [NSString stringWithFormat:@"“%@”",story.title];
...
-(void)hidePopoverLayer:(NSNotification*)notification{
    [self.popover dismissPopoverAnimated:YES];
}
@end
