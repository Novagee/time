...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deleteStory:)
                                                 name:@"DeleteStoryNotificaiton"
                                               object:nil];
    ...
    [api viewProfileWithID:TEST_USER_ID success:^(id successResponse) {
        ...
        [Session sharedInstance].user = profile;
    } failure:^(id failureResponse, NSError *error) {
        NSLog(@"err:%@",failureResponse);
    }];
    
    //我的关注
    GraphAPIManager *followAPI = [GraphAPIManager sharedInstance];
    [followAPI getFollowees:TEST_USER_ID
                    ...
                    success:^(id successResponse) {
                        ...
                        if ([successResponse isKindOfClass:[NSArray class]]) {
                            self.followingLabel.text = [NSString stringWithFormat:@"关注 %lu",(unsigned long)((NSArray*)successResponse).count] ;
                        }
                    } failure:^(id failureResponse, NSError *error) {
                        NSLog(@"err:%@",failureResponse);
                    }];
    ...
    //我的粉丝
    [followAPI getFollowers:TEST_USER_ID start:[NSNumber numberWithInt:0] limit:[NSNumber numberWithInt:10]
                    filter_followed:NO filter_unfollowed:YES success:^(id successResponse) {
        //        NSLog(@"resp:%@",successResponse);
        if ([successResponse isKindOfClass:[NSArray class]]) {
            self.followerLabel.text = [NSString stringWithFormat:@"粉丝 %lu",(unsigned long)((NSArray*)successResponse).count] ;
        }
    } failure:^(id failureResponse, NSError *error) {
        NSLog(@"err:%@",failureResponse);
    }];
    
    TimelineAPIManager *time_api = [TimelineAPIManager sharedInstance];
    [time_api getTimelineWithUser:TEST_USER_ID storyId:@"" limit:[NSNumber numberWithInt:10] success:^(id successResponse) {
        //        NSLog(@"resp:%@",successResponse);
        if ([successResponse isKindOfClass:[NSArray class]]) {
            self.stories = [NSMutableArray arrayWithArray:[Story objectArrayWithKeyValuesArray:successResponse]];
...
-(void)deleteStory:(NSNotification*)notification{
    NSDictionary* userInfo = notification.userInfo;
    NSString* story_id = (NSString*)userInfo[@"story_id"];
    for (int i;i<self.stories.count;i++) {
        Story* story = [self.stories objectAtIndex:i];
        if([story_id isEqualToString:story.story_id]){
            [self.stories removeObjectAtIndex:i];
            NSIndexPath *indexPath = [NSIndexPath indexPathForRow:i inSection:0];
            [self.tableView deleteRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationLeft];
            
            [[NSNotificationCenter defaultCenter]
             postNotificationName:@"HidePopoverLayer"
             object:self
             userInfo:nil];
