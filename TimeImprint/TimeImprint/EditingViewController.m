...
- (void)viewDidLoad {
    ...
    if ([CameraView shared].storyTitle) {
        editingView.titleTextfield .text = [CameraView shared].storyTitle;
        [CameraView shared].storyTitle = @"";
    }
...
-(void)tappedOnPublish:(UITapGestureRecognizer *)sender {
    ...
    if (selectedImageArray && selectedImageArray.count == 4) {
        ...
        [self uploadDate:UIImagePNGRepresentation([selectedImageArray objectAtIndex:0]) withType:@"Picture" OSSData:uploadData1];
        ...
        [self uploadDate:UIImagePNGRepresentation([selectedImageArray objectAtIndex:1]) withType:@"Picture" OSSData:uploadData2];
        ...
        [self uploadDate:UIImagePNGRepresentation([selectedImageArray objectAtIndex:2]) withType:@"Picture" OSSData:uploadData3];
        ...
        [self uploadDate:UIImagePNGRepresentation([selectedImageArray objectAtIndex:3]) withType:@"Picture" OSSData:uploadData4];
        self.uploadCount = 5;
    } else {
        self.uploadCount = 1;
    }
    
    yourUploadObjectKey = [NSString stringWithFormat:@"userid_video_%ld",(long)self.publishedTime];
    ...
    [self uploadDate:[NSData dataWithContentsOfURL:[CameraView shared].camRollURL] withType:@"Picture" OSSData:uploadData];
    ...
    [self uploadDate:UIImagePNGRepresentation([CameraView shared].camRollThumbNail) withType:@"video" OSSData:uploadDataVideoPreview];
}
