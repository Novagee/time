...
-(void)viewUserNotificationSuccess:(APISuccessBlock)success
                           failure:(APIFailureBlock)failure {
    [APIManager getFromPath:[NSString stringWithFormat:@"%@", USER_NOTIFICATION] body:@{} success:^(id successResponse) {
        if([successResponse isKindOfClass:[NSDictionary class]]) {
            success(successResponse);
        } else{
            NSDictionary *detail = @{NSLocalizedDescriptionKey:kResponseNotDictoryError};
...
-(void)changePassword:(NSString *)newPassword
    ...
    [APIManager putToPath:[NSString stringWithFormat:@"%@/%@",USER_PROFILE,userID] body:@{@"password":newPassword} success:^(id successResponse) {
        if ([successResponse isKindOfClass:[NSDictionary class]]) {
            success(successResponse);
        } else {
            NSDictionary *detail = @{NSLocalizedDescriptionKey:kResponseNotDictoryError};
