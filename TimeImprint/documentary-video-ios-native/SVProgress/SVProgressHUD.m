...
+ (void)setFont:(UIFont *)font {
    [self sharedView];
    SVProgressHUDFont = [UIFont fontWithName:@"Gillsans" size:14];
}
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    [self moveToPoint:CGPointMake(self.frame.size.width/2, self.frame.size.height/2) rotateAngle:rotateAngle];
    [self.hudView setNeedsDisplay];
...
- (void)showProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    if(self.alpha != 1 || self.hudView.alpha != 1) {
        ...
        self.hudView.transform = CGAffineTransformMakeRotation((90.0f * M_PI) / 180.0f);
